...
namespace Yoq.WindowsWebAuthn.Pinvoke
{
    ...
    internal class RawCommonAttestation
    {
        ...
        {
            var algoStr = (HashAlgorithm == IntPtr.Zero ? null : Marshal.PtrToStringUni(HashAlgorithm)) ?? "ES256";
            if (CoseAlgorithm == 0) CoseAlgorithm = CoseAlgorithm.ECDSA_P256_WITH_SHA256;

            var signature = new byte[SignatureBytes];
            if (SignatureBytes > 0) Marshal.Copy(Signature, signature, 0, SignatureBytes);
            ...
            if (CertInfoBytes > 0) Marshal.Copy(CertInfo, certInfo, 0, CertInfoBytes);
            var pubArea = new byte[PubAreaBytes];
            if (PubAreaBytes > 0) Marshal.Copy(PubArea, pubArea, 0, PubAreaBytes);
