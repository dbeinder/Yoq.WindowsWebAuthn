...
namespace Yoq.WindowsWebAuthn.Pinvoke.Extensions
{
    ...
    public class CredProtectExtensionIn : WebAuthnCreationExtensionInput
    {
        ...
        public CredProtectExtensionIn(UserVerification userVerification = UserVerification.OptionalWithCredentialIdList, bool requireCredProtect = true)
        {
            UserVerification = userVerification;
            RequireCredProtect = requireCredProtect;
        }

        internal override RawWebAuthnExtensionData GetExtensionData() => new RawCredProtectDataIn { UserVerification = UserVerification, RequireCredProtect = RequireCredProtect };
    }
    ...
    public class CredProtectExtensionOut : WebAuthnCreationExtensionOutput
    {
        public override ExtensionType Type => ExtensionType.CredProtect;
        public UserVerification UserVerification;
        static CredProtectExtensionOut() => Register(ExtensionType.CredProtect, r =>
            {
                var uv = UserVerification.Any;
                if (r.ExtensionDataBytes > 0)
                    uv = Marshal.PtrToStructure<RawCredProtectDataOut>(r.ExtensionData).UserVerification;
